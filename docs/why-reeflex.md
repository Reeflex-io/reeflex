...
| Split-batch evasion | not addressed | not addressed | not addressed | **durable cumulative per-session ledger — fragmentation buys nothing** (survives restarts; replicas must share the ledger volume — see [INSTALL](https://github.com/Reeflex-io/reeflex/blob/main/INSTALL.md#the-session-ledger-must-be-shared-and-must-survive-a-restart)) |
