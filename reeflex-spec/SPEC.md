...
**Adapter responsibility, second half: `agent.id` MUST distinguish the instance.** Condition 3 is worth exactly as much as the identity the adapter declares. An adapter that fills `agent.id` with a value shared by every instance of its kind — a product name, a plugin slug, a hostname — makes the comparison vacuous: every agent behind that gate presents the same actor key, condition 3 compares equal, and any of them can spend any other's approval while an implementation that checks condition 3 perfectly reports nothing wrong. So: an adapter MUST NOT emit an `agent.id` that is constant across instances sharing a gate. Where it has a durable per-agent identity (a configured client credential, a service account) it MUST use that, and it then keeps the restart tolerance condition 3 grants a named agent. Where it does not, it MUST derive the id from the session it already supplies, or omit `agent.id` entirely and let condition 3's session fallback bind — and an adapter that derives the id from the session has, by that choice, opted out of restart tolerance and SHOULD say so where the choice is made.

This is the failure mode that hides best, because nothing about it is visible from inside core or from a test that writes its own envelopes: the guard, its tests and its audit lines all behave exactly as designed.
