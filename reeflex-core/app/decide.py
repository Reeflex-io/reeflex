        ...
        with session_guard(session_id):
            ...
            if (
                ...
                try:
                    ...
                    # very different operator situations, and the commonest of
                    # them was not a defect in the store at all -- it was a
                    # second replica appending to the same volume, which now
                    # cannot cause this branch to be taken (see appendlog.py).
                    # What is left is worth telling apart:
                    #
                    #   hold_store_integrity     the record could not be proven
                    #                            to have landed intact.
                    #                            Investigate the volume; the log
                    #                            may be torn or rewritten.
                    #                            PAGE SOMEBODY.
                    #   hold_store_unavailable   core could not write/lock at
                    #                            all (disk full, volume
                    #                            read-only, lock file
                    #                            unopenable).  An availability
                    #                            problem; it says nothing about
                    #                            integrity.
                    #   hold_creation_failed     retained for anything else, so
                    #                            no existing consumer of this id
                    #                            breaks.
                    ...
                    if cause == "tampered":
                        ...
                        denial["reason"] = (
                            "hold record could not be proven to have landed "
                            "intact - failing closed"
                        )
