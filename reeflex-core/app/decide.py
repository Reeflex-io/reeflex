        ...
        with session_guard(session_id):
            ...
            if (
                ...
                try:
                    ...
                    #                            breaks.
                    #
                    # MERGE NOTE (dev-3 round 039). This block is RFX-207's
                    # (#113) and this location is RFX-197's (#110). #110 moved
                    # hold creation out of the Step 7 try-body and into a Step 9
                    # block nested inside `with session_guard(...)` and the
                    # `if require_approval` test, so git aligned #113's handler
                    # against #110's OPA-eval handler and offered a choice
                    # between the two that would have deleted one of them:
                    # taking "ours" drops #110's restructure and duplicates the
                    # hold block, taking "theirs" drops the three-way cause
                    # dispatch and re-opens RFX-207. Neither PR touches the
                    # other's subject. Pinned by
                    # tests/test_append_readback_rfx207.py::ARefusalNamesItsCause
                    # (5 tests), which is the check on this resolution.
