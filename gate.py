...
def score_entrypoint_help(entry, has_usage, code, out):
...
class Gate:
    ...
    def run_entrypoints(self):
        ...
        for pkg, entry, has_usage in PUBLISHED:
            ...
            ok, detail = score_entrypoint_help(entry, has_usage, code, out)
            if ok:
...
def selftest():
    ...
    check("entrypoint scorer accepts exit 0 + anchored banner",
          score_entrypoint_help("reeflex-holds", True, 0, _USAGE_OUT)[0])
    check("entrypoint scorer REJECTS exit 0 with no output at all",
          not score_entrypoint_help("reeflex-holds", True, 0, "")[0])
    check("entrypoint scorer names the silent-banner failure, not the exit code",
          "no anchored 'usage: reeflex-holds' banner"
          in score_entrypoint_help("reeflex-holds", True, 0, "")[1])
    check("entrypoint scorer rejects the banner as a mid-line substring",
          not score_entrypoint_help("reeflex-holds", True, 0,
                                    "error: bad usage: reeflex-holds [-h]\n")[0])
    check("entrypoint scorer rejects a nonzero exit despite a banner",
          not score_entrypoint_help("reeflex-holds", True, 1, _USAGE_OUT)[0])
    check("entrypoint scorer allows exit 0 with no banner only where none is declared",
          score_entrypoint_help("some-daemon", False, 0, "")[0])
    check("entrypoint scorer says so when it accepted a script with no banner",
          "nothing more" in score_entrypoint_help("some-daemon", False, 0, "")[1])
