...
- **A human's approval now authorises THAT AGENT, not just that action (RFX-138, RFX-139).** An approval was bound to the action alone, so any other agent — or the same agent claiming to act for a different person — could spend it. Measured, not reasoned: agent ALPHA raised an irreversible production delete, a human approved **ALPHA's** request, agent BETA resubmitted with ALPHA's `hold_id` and got `allow` / `reeflex.policy/approved_resubmission`, and ALPHA — the only agent a human ever approved — was then refused `reeflex_hold_consumed`. So the action executed for an agent no human saw *and* the approved agent was locked out. The `on_behalf_of` variant is worse because it leaves no trace: same bot, same `agent.id`, same `session_id`, `alice` → `bob`, and core's audit line for the allow is byte-identical to a legitimate resubmission. Confirmed on live api-dev v0.1.13 **and** on `origin/main` 44c6f85 — i.e. after #92/#93 added check 7 — and reachable by one environment variable (`reeflex-claude` reads `on_behalf_of` from `REEFLEX_CLAUDE_PRINCIPAL` in the agent's own process).
  ...
  - **The mechanism, not the patch.** The `agent` block joins the approval-bound set, so `agent.id`, `agent.on_behalf_of` and `agent.session_id` are bound to the hold, and a substitution is denied `reeflex_hold_actor_substituted` — a distinct reason, because on a substitution the action matched *perfectly* and only the actor moved; reporting that as `envelope_mismatch` would name the one thing that is not true. Identities compare through `principal.normalize_identity()`, the same fold check 6 and the resolve-time guard use, so `svc-bot` vs `SVC-BOT` is the same actor rather than a false deny. `envelope_hash`'s preimage is **deliberately unchanged** — audit, SIEM, hold records and the evidence chain all join on it. And the guard cannot be made vacuous by omitting `agent.id` (SPEC §2 does not require it): `session_id` is required and non-empty, so the bound identity set is never empty — the RFX-CORE-2/A2 lesson applied to binding rather than four-eyes.
  ...
  - A7 in `reeflex-core/tests/test_envelope_boundary_attacks.py` (8 tests; **6 fail on 44c6f85**, the other 2 are controls that must pass on both) and a matching A7 row in `scripts/attack-probe-rfx97-release-gate.py`, which reports `STILL EXPLOITABLE 2/3` against unfixed main and `CLOSED 0/3` here. **That row's absence was a false all-clear:** the gate printed "all five closed" and exited 0 against the published v0.1.13 image while this was live on it.
