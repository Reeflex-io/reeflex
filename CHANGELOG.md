...
- **The Claude Code adapter priced the agent's own phrasing instead of the action — 24 of 27 irreversible PRODUCTION destructions were allowed with no human, and no rule in core could see it (RFX-144, RFX-145, RFX-146).** `reeflex_claude/classify.py::_bash_verb()` took `os.path.basename(shlex.split(command)[0])` — the FIRST token of the command line — so `cd /srv/prod && rm -rf data` was priced as a `cd`, `echo cleaning up && rm -rf /srv/prod/data` as a read, and `sh -c 'rm -rf /srv/prod/data'` as a `sh`. Core observes none of the axes (SPEC §6), so a wrong axis here is a wrong decision there. **Measured through the shipped hook against a real core built from `main@44c6f85`: 24 of 27 canonical irreversible production destructions → `allow`, 16 of them exactly as RFX-144 enumerates them; 0 of 22 ordinary developer commands blocked.** After: **0 and 0.** `scripts/attack-probe-rfx144-adapter-classification.py` is that walk, exit code = the number still exploitable, and `--budget` adds RFX-146's two 30-call sessions.
  ...
  - `reeflex-claude/tests/test_classify_rfx144.py` — 48 tests asserting on the AXES (R2/R3's body) rather than on the verb, because a verb-only assertion would have gone green on main while the decision stayed `allow`. **49 of them fail against `main@44c6f85`'s `classify.py`; the 17 that pass are the no-regression and over-block controls, which must be green both ways.** `test_classify.py::test_command_chaining_ls_rm` previously asserted only that `ls; rm -rf /` produced one of the four legal verbs and one of the four legal tiers — an assertion that could not fail, documenting the fail-open as intended behaviour. It now asserts the outcome.
