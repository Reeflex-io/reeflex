...
- **`REEFLEX_REQUIRE_VERIFIED_APPROVER` now defaults to `true`, and `reeflex-core` is `0.2.0` because of it (RFX-84, RFX-97).**
  ...
  **Not fixed here, and deliberately not "fixed" by weakening the check:** RFX-179 — the RFX-97 release-gate probe holds one bearer token, so at the new default its A3-self and A6 rows are refused `principal_not_verified` before reaching the guards they name and score `INCONCLUSIVE`. That is the probe's limitation, not the artefact's; per `gate-inconclusive-does-not-fail-the-exit-code` an `INCONCLUSIVE` row still lets the runner print `still exploitable: none` and exit 0, so the flag-on run must be read with that caveat. Both guards are covered by unit tests and by WP H10 end to end. RFX-179 is claimed by dev-1.
