...
### Fixed
- **`reeflex-claude check` can now fail for the reason the customer cares about (RFX-147).** The README tells a new customer to wire and verify in two commands. The second one, `check`, probed exactly one thing: point the hook at an *unreachable* core and assert it denies. That property is real — the fail-closed invariant is the best-built thing in the package — and it was the ONLY thing checked, so `check` printed `PASS -- fail-closed verified`, exit 0, on an installation where `kubectl delete namespace production` was **allowed with no human**. Its exit code was a statement about the network. Reproduced before the fix against a core built from `44c6f85` (`code-reports/dev-1--021-evidence/01-before-main-44c6f85.txt`): the same core answers `allow` for `kubectl delete namespace production` and `check` still prints PASS. `check` now runs a **second stage** — a named set of destructive payloads (`reeflex_claude/gate_probe.py`: recursive delete, delete behind `cd &&`, delete behind `echo &&`, cluster-object delete, IaC destroy, cloud-bucket recursive delete, plus one benign control) sent through the hook to the operator's **real configured core**, with a per-payload verdict printed either way and a **non-zero exit on an unexpected allow**. Nothing is executed: a PreToolUse hook only classifies a proposed tool call. Three things make the new verdict honest rather than merely louder. (1) **A deny is not automatically a pass** — an unreachable core denies *everything*, which looks identical to a working gate, so each verdict is judged on its **rule**: only `reeflex.policy/...` is a decision the policy pack made, while `reeflex.core/fail_closed` is reported `NOT-EXERCISED` and the headline says `GATE NOT VERIFIED`. Same for `REEFLEX_MODE=observe`, which allows by design. (2) **The probes run under the configuration the hook will actually run under** — the `env` block from `settings.json` is applied on top of the shell environment, exactly as Claude Code does, and the effective core URL / mode / environment are printed before the probes; a check built only from the shell can otherwise make a confident statement about a configuration the hook never sees. (3) **The asymmetry is deliberate**: an unexpected *allow* on a destruction fails, an unexpected *deny* of the benign control only warns — an operator running a stricter pack than the base canon must not have their check fail for being strict, while the control still catches the degenerate gate that denies everything. `--require-gate` makes "the gate could not be verified" a failure (for CI); `--skip-gate` runs stage 1 only and says so. Unlike `REEFLEX_CLAUDE_STRICT` (RFX-145), `--require-gate` is proved to **move an exit code** on one unchanged installation — `tests/test_gate_probe.py` (25 tests) pins that, the whole outcome table, and specifically that a `reeflex.core/fail_closed` deny is not counted as a working gate. **The check is also proved satisfiable**, which matters because a check that can only ever fail is indistinguishable from a hardcoded FAIL: overlaying the in-flight RFX-144 classifier turns the same run green, 6 of 6 gated, benign control still allowed (`03-satisfiable-with-rfx144-fix.txt`). **And it now RUNS**: `gate.yml` gains a `claude-gate-probe` job that starts a core from the tree, `pip install`s the adapter the way a customer does and runs `reeflex-claude check --require-gate`. **That job is RED on `main` today, correctly** — the first-token verb rule lets 5 of the 6 destructive probes through (RFX-144/RFX-146, fix in flight). It is a separate job precisely so an honest red does not block every unrelated PR on someone else's fix; **whether it becomes a required check is an owner decision.**
