...
Core decision traceability + concurrency-safe hold consumption — the dependency baseline for the `reeflex-mcp` gateway. No decision verdict or decision-logic change; additive only. (Core version realigns to the release tag here: 0.1.9 and 0.1.10 were adapter/channel releases where `reeflex-core` was unchanged, so core skipped them.)
