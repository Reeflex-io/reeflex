...
### Fixed
- **The Claude Code adapter priced the agent's own phrasing instead of the action — 24 of 27 irreversible PRODUCTION destructions were allowed with no human, and no rule in core could see it (RFX-144, RFX-145, RFX-146).** `reeflex_claude/classify.py::_bash_verb()` took `os.path.basename(shlex.split(command)[0])` — the FIRST token of the command line — so `cd /srv/prod && rm -rf data` was priced as a `cd`, `echo cleaning up && rm -rf /srv/prod/data` as a read, and `sh -c 'rm -rf /srv/prod/data'` as a `sh`. Core observes none of the axes (SPEC §6), so a wrong axis here is a wrong decision there. **Measured through the shipped hook against a real core built from `main@44c6f85`: 24 of 27 canonical irreversible production destructions → `allow`, 16 of them exactly as RFX-144 enumerates them; 0 of 22 ordinary developer commands blocked.** After: **0 and 0.** `scripts/attack-probe-rfx144-adapter-classification.py` is that walk, exit code = the number still exploitable, and `--budget` adds RFX-146's two 30-call sessions.
  - **The parse.** The command string is now DECOMPOSED before anything is classified: quote-aware split on `;` `&&` `||` `|` `&` and newline, leading `VAR=value` assignments and prefix wrappers (`env`, `sudo`, `nohup`, `timeout N`, `nice`, `xargs`, …) stripped, and `sh|bash|zsh|dash -c '<string>'` recursed into up to `_MAX_UNWRAP_DEPTH`. Every segment is classified and the most severe result decides. The quote-awareness is load-bearing in both directions: it is also what keeps `python3 -c "a; b"` ONE segment, so an inline program is judged as an inline program instead of as a harmless `python3` followed by a `b`. This layer makes no claim about what any command means — only about where one ends and the next begins.
  - **The default, which was the larger half.** An unrecognised command was classified `execute / recoverable / scoped` — a *measurement* asserting the action is undoable and bounded, made about a command the adapter had never seen. For `kubectl delete namespace production` that is false in the direction that allows. The module's own AXIS MAPPING section has always stated SPEC §2's conservative defaults (unknown reversibility → irreversible) and did not apply them here; it does now, as a named **UNRESOLVED** class: `irreversible` + `broad`, `danger_signature: unclassified_command`. `broad` and not `systemic` is deliberate — `broad` routes to R2 (a human decides), `systemic` to R3 (refused even with approval). An adapter that cannot identify a command has established that nobody checked, not that the command is catastrophic. **This does not pre-empt RFX-132**, the open owner-level question about core's own DENY-vs-HOLD default.
  - **The direction of the list is the fix.** What keeps this from prompting on every build is `_DEV_SAFE`, a positive list of recognised non-destructive developer operations, plus `_CLOUD_READ_OP_RE` for the cloud CLIs (whose safe surface is an operation vocabulary, not an enumerable set of services). A DENYLIST of dangerous commands fails OPEN on the command nobody thought of — which is how `terraform destroy -auto-approve` was allowed. An ALLOWLIST of safe commands fails NOISY: a missing entry costs an operator a prompt, with a visible symptom, instead of a silent production deletion.
  - **Resource deletes are deletes, and cardinality is still not read off a name.** `kubectl delete`, `aws s3 rm`, `aws <svc> delete-*`, `gcloud … delete`, `az … delete`, `docker <obj> rm`, `terraform destroy`, `helm uninstall`, `find … -delete` and truncating overwrites (`> db.sqlite`, `dd of=`, `mkfs`) now classify as `delete`. `blast_radius` comes from the shape of the affected set, consistent with RFX-131's rule: a CONTAINER kind (namespace, volume, database, bucket, cluster, block device) → `systemic`; an unbounded selector (`--all`, `--recursive`, a label selector) → `broad`; an enumeration → cardinality, same inclusive boundary at 20. `kubectl delete pod a b` is `scoped`/2, not `broad`.
  - **RFX-146 — the cumulative budget was blind to the same laundering.** `budgets.rego`'s `deletions` dimension counts `input.action.verb == "delete"`, so a destruction classified `execute` never accumulated however often it was repeated. Measured before: 30 `rm` in one session held at call 21, 30 `kubectl delete namespace` and 30 `aws s3 rm` **never held in 30 calls**. After: `aws s3 rm` holds at 21 alongside `rm`, and `kubectl delete namespace` is refused at call 1 by R3. No change to core or to the policy pack.
  - **RFX-145 — `REEFLEX_CLAUDE_STRICT`, the only documented knob for tightening the adapter, changed no decision at all.** Its sole effect was flipping `reversibility` on a class whose `blast_radius` was `scoped`, and R2 requires `broad`, so the same 24 fail-opens were allowed with it set and unset. It now raises the UNRESOLVED class from `broad` to `systemic` — from "a human must approve this" to "this does not run" — which is what an operator setting a tightening flag is asking for. Measured: `psql -h prod-db -f /tmp/migrate.sql` and `python3 -c '<program>'` move `ask` → `deny`, and 0 of 22 ordinary commands are blocked either way. It still also marks recognised developer operations `irreversible`, which on the stock pack changes no verdict on its own — stated in the README rather than quietly dropped.
  - **Residue, named rather than hidden.** A single-entity irreversible delete in production (`rm /tmp/scratch.txt`, `dd of=/tmp/noise`) is `single` and the stock pack allows it via R4 default-allow. Forcing it to `broad` to make this ticket's number zero would be exactly the name-derived cardinality claim RFX-131 forbids; the gap is R4's shape and it is **RFX-128's** ticket. Command substitution (`$(...)`), process substitution, `eval` of a constructed string and here-docs are not expanded, so a destruction assembled at runtime lands in UNRESOLVED — the intended failure direction, but a failure to *identify* and not a defence. Both are written into `reeflex-claude/README.md`'s limits section.
  - **`ssh host '<command>'` was allowed too, and that one was found by the other agent working the same cluster.** `_EMIT_RE` matched `ssh` before anything else and the EMIT class emits `blast_radius: scoped`, so `ssh prod 'rm -rf /srv/data'` could not reach R2 — remote command execution wearing an emit hat. dev-2 surfaced it as RFX-158's `gap-remote-execution` row while measuring their own branch (PR #99) for these same tickets. The remote command string is now classified the way `sh -c`'s is, the worse of the two readings wins, and `externality` stays `outbound` because the bytes do leave. `ssh prod ls` is still `allow`; a bare `ssh prod` is UNRESOLVED, because an interactive channel this hook cannot see into and will never be asked about again is exactly what that class is for. Leaving a known member of the class open in a change that claims to close the class would have made the claim false.
  - `reeflex-claude/tests/test_classify_rfx144.py` — 55 tests asserting on the AXES (R2/R3's body) rather than on the verb, because a verb-only assertion would have gone green on main while the decision stayed `allow`. **all 55 fail against `main@44c6f85`'s `classify.py` (20 of the individual subtest rows inside them pass — the no-regression and over-block controls, which must be green both ways).** `test_classify.py::test_command_chaining_ls_rm` previously asserted only that `ls; rm -rf /` produced one of the four legal verbs and one of the four legal tiers — an assertion that could not fail, documenting the fail-open as intended behaviour. It now asserts the outcome.
