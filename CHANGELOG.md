...
### Fixed
- **A human's approval now authorises THAT AGENT, not just that action (RFX-138, RFX-139).** An approval was bound to the action alone, so any other agent — or the same agent claiming to act for a different person — could spend it. Measured, not reasoned: agent ALPHA raised an irreversible production delete, a human approved **ALPHA's** request, agent BETA resubmitted with ALPHA's `hold_id` and got `allow` / `reeflex.policy/approved_resubmission`, and ALPHA — the only agent a human ever approved — was then refused `reeflex_hold_consumed`. So the action executed for an agent no human saw *and* the approved agent was locked out. The `on_behalf_of` variant is worse because it leaves no trace: same bot, same `agent.id`, same `session_id`, `alice` → `bob`, and core's audit line for the allow is byte-identical to a legitimate resubmission. Confirmed on live api-dev v0.1.13 **and** on `origin/main` 44c6f85 — i.e. after #92/#93 added check 7 — and reachable by one environment variable (`reeflex-claude` reads `on_behalf_of` from `REEFLEX_CLAUDE_PRINCIPAL` in the agent's own process).
  - **Why check 7 did not already cover it, which is the part worth keeping.** Check 7 iterates `approval_bound_paths()`, derived from `TREATMENTS` filtered by a bound-block set that excluded `agent` with the reasoning *"not decision inputs to a rule"*. That sentence is true and it was the wrong test: `agent.id` and `agent.on_behalf_of` are not inputs to a RULE, they are **who the human said yes to** — and check 6 reads both, so they are demonstrably inputs to the approval chain check 7 belongs to. Underneath sat **RFX-139**: neither field was declared in `TREATMENTS` at all, so no filter over the declarations could ever have returned them. `DECIDE_ENVELOPE_PATHS` named four paths while `decide.py` read six, and the guard asserting "nothing undeclared" passed **by under-reporting**. The two missing reads happen one frame deeper, inside `principal.actor_identities()`, behind `for field in ("id", "on_behalf_of", "session_id")` — so `app/principal.py` is exactly the *fourth reader* `field_treatments.py`'s own residual note predicted would be invisible.
  - **The mechanism, not the patch.** The `agent` block joins the approval-bound set, and is compared as **one ordered actor key** (`principal.approval_actor_key()`) rather than field by field. A substitution is denied `reeflex_hold_actor_mismatch` — a distinct reason, because on a substitution the action matched *perfectly* and only the actor moved; reporting that as `envelope_mismatch` would name the one thing that is not true, and qa's finding was that the substitution left no trace. Identities compare folded through `principal.normalize_identity()`, the same fold check 6 and the resolve-time guard use, so `svc-bot` vs `SVC-BOT` is the same actor rather than a false deny. `envelope_hash`'s preimage is **deliberately unchanged** — audit, SIEM, hold records and the evidence chain all join on it. And the guard cannot be made vacuous by omitting `agent.id` (SPEC §2 does not require it): `session_id` is the fallback when the envelope names no agent at all, so the key is never empty — the RFX-CORE-2/A2 lesson applied to binding rather than four-eyes.
  - **`agent.session_id` is a fallback, not a co-equal, and that distinction was measured rather than reasoned.** A hold lives 4h by default, so an agent that **restarts** between raising it and resubmitting presents the same `agent.id` and the same `on_behalf_of` with a new session. Binding all three uniformly denies that — a wrong deny on the one path in the product where a human explicitly said yes, and a regression against `main`, which allows it. An earlier version of this fix did exactly that; it is now `test_a7_a_restarted_agent_does_not_lose_an_approval_a_human_granted`, and the RFX-97 gate's A7 row scores it as an evasion so a release cannot ship the over-tight version either. The security cost of the exclusion is ~nil: an attacker who must already present the same `agent.id` **and** the same `on_behalf_of` is that agent as far as core can tell. Credit to dev-1, who reached the same conclusion independently on `dev-1/RFX-138-approval-binds-the-actor`.
  - **The derivation survives the special case.** Carving a block out of a derived loop is itself the RFX-139 shape, so `field_treatments.actor_bound_paths()` declares which `agent.*` paths the actor key must read, and `test_the_actor_key_reads_every_declared_actor_path` drives the recording envelope through `approval_actor_key()` to check it does. Declare `agent.tenant_id` tomorrow and it goes red until the key reads it.
  - **Nothing was asked of adapters that they were not already doing.** The WordPress adapter already replays `{id, on_behalf_of, session_id}` verbatim on a resubmission and calls it a *"LOCKED DECISION … the actor stays the actor"*; `reeflex-mcp`'s holds tracker is keyed on `(session_id, action_hash)`, so a cross-session resubmission cannot even find the hold; the n8n recipe respreads the whole envelope and regenerates only `meta.nonce`. Core now enforces at its boundary an invariant every reference adapter already promised. SPEC §5.1 gains a normative *"what an approval authorises"* subsection stating it as four conditions.
  - **How RFX-139 stops recurring.** `tests/test_field_treatments.py` drives a **recording envelope** through the real approval chain and asserts that every path the chain actually dereferences is declared. The ticket proposed an AST sweep; an AST sweep of `decide.py` would **not** have caught this, because there is no `.get("on_behalf_of")` literal anywhere in the scanned file. On its first run the probe found a *second* instance nobody had noticed — `params.amount` and `params.currency` are read by check 7 and were missing from `DECIDE_ENVELOPE_PATHS` too, staying green only because `ledger.py` also declares them.
  - A7 in `reeflex-core/tests/test_envelope_boundary_attacks.py` (9 tests; **6 fail on 44c6f85**, the other 2 are controls that must pass on both) and a matching A7 row in `scripts/attack-probe-rfx97-release-gate.py`, which reports `STILL EXPLOITABLE 2/4` against unfixed main, `STILL EXPLOITABLE 1/4 (WRONG DENY)` against the over-tight version, and `CLOSED 0/4` here. **That row's absence was a false all-clear:** the gate printed "all five closed" and exited 0 against the published v0.1.13 image while this was live on it.
